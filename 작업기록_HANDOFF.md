...
### 🚨 (H) 데이터 유실 사고 & 복구 (2026-07-24 08:17) — 반드시 읽을 것
- **사고:** Firestore의 회원·묵상·공지·QnA가 전부 삭제됨(회원 4명 → 1명)
- **원인:** `db.json`을 GitHub에서 제외(보안상 올바른 조치)했는데, 이 앱에 결함이 있었음 —
  `db.json` 없는 새 서버 기동 → 회원 0명 "초기 DB" 생성 → Firestore 읽기 실패 →
  **`server.ts`의 `saveToFirestore(빈 DB)`가 원격을 통째로 덮어씀**
- **복구:** 로컬 `db.json` 백업(06:03 시점)으로 전량 복구 완료 (회원 4명·공지 6·QnA 5·감사 2·묵상 1·통독진도 4명분)
- **백업 파일 위치:** `C:\Users\PC\Downloads\bible-meditation-share_DB백업_20260724.json`
...
- **검증:** `db.json` 없는 상태로 기동 → `[SAFETY] ... 쓰기 차단` 로그 후 Firestore에서 회원 4명 정상 복원 확인
- ⚠️ **교훈:** 이 앱은 전체 DB를 Firestore 단일 문서에 통째 덮어쓰는 구조라 위험. **배포 전 항상 `db.json` 백업**을 떠둘 것.

...
import { Heart, MessageSquare, Send, Trash2, Sparkles, UserCheck, EyeOff, Calendar, Search, Plus, X, Radio } from "lucide-react";
...
export default function DailyGratitude({ currentUser }: DailyGratitudeProps) {
  ...
  const filteredGratitudes = gratitudes.filter(g => {
    ...
    if (searchTerm.trim()) {
      ...
      const matchContent = g.content.toLowerCase().includes(term);
      const matchAuthor = !g.isAnonymous && g.userName.toLowerCase().includes(term);
      const matchDate = g.date.includes(term);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <h2 className="text-xl font-bold tracking-tight">오늘의 감사 나눔터</h2>

  ...
  return (
    <div className={compact ? "" : "space-y-1.5"}>
      <button
        ...
        } ${open ? "bg-[#0C342C] text-white" : "bg-[#EEF5E1] text-[#1E6B57] hover:bg-[#E2EBD6]"}`}
      >
      ...
      {open && (
        <div className="flex flex-wrap gap-1.5 bg-[#EEF5E1] rounded-3xl p-2.5">
          {names.map((name) => (
            <button
              key={name}
              type="button"
              ...
              className="px-2.5 py-1.5 rounded-3xl bg-white text-2xs font-bold text-[#0C342C] hover:bg-[#E7F6D8] ring-1 ring-[#DEE9D2] transition cursor-pointer"
            >
              @{name}
            </button>
          ))}
          <p className="basis-full text-2xs text-[#4E7568] px-1 pt-0.5">

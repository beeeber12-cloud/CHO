...
export default function AppGuide({ onClose, isAdmin = false }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-[#0C342C] flex flex-col text-white">
      {/* 목차 — 누르면 그 장으로 바로 간다 */}
      <div className="shrink-0 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <div className="flex items-center gap-2 px-4">
            ...
            {chapters.map((c, ci) => (
              <button
                ...
                className={`shrink-0 px-3 py-1.5 rounded-full text-2xs font-bold transition cursor-pointer ${
                  ci === cur.ci
                    ? "bg-white text-[#0C342C]"
                    : "bg-[#0A4A3E] text-[#7C9A87] hover:text-white"
                }`}
              >
                {c.name}
              </button>
          ...
          <button
            onClick={finish}
            className="shrink-0 text-2xs font-bold text-[#7C9A87] hover:text-white px-2 py-1.5 cursor-pointer"
          >
            건너뛰기
          </button>
        </div>
      ...
      >
        <div ref={dragRef} className="w-full max-w-md py-6">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="w-14 h-14 rounded-3xl bg-[#0A4A3E] flex items-center justify-center mb-5">
                <Icon size={26} className="text-white" />
              </div>

              <span className="block text-2xs font-black tracking-[0.15em] text-[#E3EF26] mb-1.5">
                {cur.chapter.name}
              </span>

              {/* 장의 첫 단계에서는 이 화면이 무엇을 하는 곳인지 먼저 알려 준다 */}
              {cur.si === 0 && (
                <p className="text-sm text-[#7C9A87] mb-3 leading-relaxed">{cur.chapter.intro}</p>
              )}

              {/* 이 단계에서 짚는 버튼 */}
              {cur.step.button && (
                <span className="inline-block bg-[#0A4A3E] text-white text-xs font-bold rounded-full px-3 py-1.5 mb-3">
                  {cur.step.button}
                </span>
              )}
              ...
              <div className="space-y-1.5">
                {cur.step.lines.map((line, k) =>
                  line === "" ? (
                    ...
                    <p key={k} className="text-[#CFE0C2] leading-relaxed">
                      {line}
                    </p>
                  )
                )}
      ...
      <div className="shrink-0 px-7 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-3 space-y-3.5">
          ...
          {cur.chapter.steps.map((_, k) => (
            <span
              ...
              className={`h-1.5 rounded-full transition-all ${
                k === cur.si ? "w-6 bg-white" : "w-1.5 bg-[#1E6B57]"
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => go(i - 1)}
            disabled={i === 0}
            className="p-3 rounded-3xl bg-[#0A4A3E] text-white disabled:opacity-30 cursor-pointer"
            aria-label="이전"
          >
            <ChevronLeft size={20} />
          </button>
          ...
          {last ? (
            <button
              onClick={finish}
              className="flex-1 py-3.5 rounded-3xl bg-white text-[#0C342C] font-bold cursor-pointer"
            >
              시작하기
            </button>
          ) : (
            <button
              onClick={() => go(i + 1)}
              className="flex-1 py-3.5 rounded-3xl bg-[#E3EF26] text-[#0C342C] font-bold flex items-center justify-center gap-1 cursor-pointer"
            >
              다음
        ...
        <p className="text-center text-2xs text-[#4E7568]">

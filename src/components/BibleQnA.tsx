...
export default function BibleQnA({ currentUser }: BibleQnAProps) {
  ...
  return (
    <div className="space-y-5">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[#06231D] via-[#0C342C] to-[#06231D] text-white rounded-[32px] p-5 shadow-sm relative overflow-hidden">
        <div className="relative z-10 space-y-3">
          <div className="flex items-center gap-2">
            <span className="p-1.5 bg-[#1E6B57]/20 text-[#FFFFFF] rounded-3xl">
              <BookOpen size={18} />
            </span>
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight">AI 성경 역사 & 배경 Q&A</h2>
          </div>

          <p className="text-xs text-[#CFE0C2]/90 leading-relaxed max-w-2xl">
      ...
      <div className="bg-white rounded-[32px] shadow-sm p-4 sm:p-5 space-y-3">
        <div className="flex items-center justify-between border-b border-[#DEE9D2] pb-2.5">
          <h3 className="text-sm font-bold text-[#0C342C] flex items-center gap-1.5">
            <HelpCircle size={16} className="text-[#1E6B57]" />
            성경 역사 및 배경 질문하기
          </h3>
          <span className="text-2xs text-[#4E7568] bg-[#EEF5E1] px-2.5 py-0.5 rounded-xl">
            🔒 익명 질문
          </span>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            <select
              ...
              className="px-3 py-2 text-xs font-semibold bg-[#EEF5E1] rounded-3xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] bg-white text-[#0B2A20] shrink-0 cursor-pointer shadow-sm"
            >
            ...
            <input
              ...
              className="flex-1 px-3.5 py-2.5 text-xs sm:text-sm font-medium bg-[#EEF5E1] rounded-3xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] bg-white text-[#0B2A20] shadow-sm"
            />
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-5 py-2.5 bg-[#0C342C] hover:bg-[#06231D] text-white font-bold text-xs rounded-3xl shadow transition cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="bg-white rounded-3xl p-3 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-2.5">
        <div className="relative w-full sm:w-72">
          <Search size={14} className="absolute left-3 top-2.5 text-[#4E7568]" />
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Q&A 내용, 단어 검색..."
            className="w-full pl-8 pr-3 py-1.5 text-xs font-medium bg-[#EEF5E1] rounded-3xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] bg-white text-[#0B2A20]"
          />
        </div>
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-3xl text-xs font-bold transition cursor-pointer ${
                selectedCategory === cat
                  ...
                  : "bg-[#EEF5E1] text-[#4E7568] hover:text-[#0C342C]"
              }`}
            >
              {cat}
            </button>
          ))}
          <span className="text-2xs text-[#4E7568] font-semibold bg-[#EEF5E1] px-2 py-1 rounded-3xl">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[32px] p-10 text-center shadow-sm space-y-3">
          ...
          <div>
            <h4 className="text-sm font-bold text-[#0C342C]">등록된 성경 질문이 없습니다.</h4>
          ...
          {filteredQnA.map((qa) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white rounded-3xl sm:rounded-[32px] shadow-sm p-4 sm:p-5 transition hover:border-[#1E6B57]/40 space-y-3.5"
              >
                {/* Header Tag and Delete */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded-full text-2xs font-bold bg-[#EEF5E1] text-[#0C342C]">
                      {qa.category || "성경 역사 & 배경"}
                    </span>
                    <span className="text-2xs text-[#4E7568] flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="p-1.5 text-[#4E7568] hover:text-[#B3261E] rounded-xl transition cursor-pointer flex items-center gap-1 text-2xs"
                    title="질문 삭제"
                  >
                    <Trash2 size={14} />
                    <span className="text-2xs text-[#4E7568] hover:text-[#B3261E] font-semibold">삭제</span>
                  </button>
                </div>

                {/* Question */}
                <div className="bg-[#EEF5E1] p-3.5 rounded-3xl flex items-start gap-2.5">
                  <span className="w-6 h-6 rounded-xl bg-[#0C342C] text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                    Q
                  </span>
                  <p className="text-xs sm:text-sm font-bold text-[#0C342C] leading-relaxed pt-0.5">
                    {qa.question}
                  </p>
                </div>
                ...
                {!isExpanded ? (
                  <button
                    ...
                    className="w-full py-2.5 px-4 bg-[#EEF5E1] hover:bg-[#EEF5E1] text-[#0C342C] rounded-3xl text-xs font-bold transition flex items-center justify-between cursor-pointer group shadow-xs"
                  >
                    <div className="flex items-center gap-2">
                      <Sparkles size={15} className="text-[#1E6B57] group-hover:scale-110 transition" />
                      <span>AI 성경 학자 & 역사 해설</span>
                    </div>
                    <div className="flex items-center gap-1 text-xs text-[#0C342C] font-extrabold bg-white px-2.5 py-1 rounded-3xl shadow-2xs">
                      <span>답변 열기</span>
                      <ChevronDown size={15} className="text-[#1E6B57]" />
                    </div>
                  </button>
                ) : (
                  <motion.div
                    ...
                    className="bg-[#EEF5E1] p-4 rounded-3xl space-y-3 relative shadow-inner"
                  >
                    {/* Answer Header */}
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#DEE9D2] pb-2.5">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-[#0C342C]">
                        <Sparkles size={15} className="text-[#1E6B57]" />
                        <span>AI 성경 학자 & 역사 해설</span>
                      </div>

                      <div className="flex items-center gap-2">
                        <span className="text-2xs bg-[#EEF5E1]/80 text-[#0C342C] px-2 py-0.5 rounded-lg font-semibold">
                          Gemini 3.6
                        </span>
                        <button
                          onClick={() => handleRegenerate(qa.id)}
                          disabled={regeneratingId === qa.id}
                          className="text-2xs bg-[#0C342C] hover:bg-[#06231D] text-white px-2 py-0.5 rounded-lg font-bold transition flex items-center gap-1 cursor-pointer disabled:opacity-50"
                        ...
                        <button
                          onClick={() => toggleExpand(qa.id)}
                          className="text-xs bg-[#EEF5E1] hover:bg-[#DEE9D2] text-[#0C342C] px-2.5 py-1 rounded-3xl font-bold transition flex items-center gap-1 cursor-pointer"
                        >
                          <span>답변 닫기</span>
                          <ChevronUp size={15} className="text-[#1E6B57]" />
                        </button>
                      </div>
                    </div>

                    {/* Full Markdown Answer Content */}
                    <div className="text-xs sm:text-sm text-[#0B2A20] leading-relaxed font-normal space-y-2">
                    ...
                    {/* Bottom Close Button */}
                    <div className="pt-2 border-t border-[#EEF5E1] flex justify-end">
                      <button
                        onClick={() => toggleExpand(qa.id)}
                        className="text-xs text-[#0C342C] hover:bg-[#EEF5E1] px-3 py-1.5 rounded-3xl font-bold flex items-center gap-1 cursor-pointer transition"
                      >
                        <span>답변 닫기</span>
                        <ChevronUp size={15} className="text-[#1E6B57]" />
                      </button>
                ...
                <div className="flex items-center justify-between pt-1 text-xs border-t border-[#EEF5E1]">
                  ...
                  {isCommentOpen && (
                    ...
                    >
                      <div className="flex items-center justify-between text-xs font-bold text-[#0C342C]">
                        ...
                        <span className="text-2xs text-[#4E7568]">질문과 답변에 대해 의견을 나눌 수 있습니다</span>
                      </div>
                      ...
                      {qa.comments.length > 0 ? (
                              ...
                              >
                                <div>
                                  <div className="flex items-center gap-1.5 mb-0.5">
                                    <span className={`font-bold ${isPastorOrAdmin ? "text-[#0C342C]" : "text-[#0C342C]"}`}>
                                      {comment.userName}
                                    </span>
                                    {isPastorOrAdmin && (
                                      <span className="text-2xs bg-[#0C342C] text-white px-1.5 py-0.2 rounded font-bold">
                                        목양 코멘트
                                      </span>
                                    )}
                                    <span className="text-2xs text-[#4E7568]">
                                  ...
                                  <p className="text-[#1E6B57] leading-relaxed">{comment.content}</p>
                                </div>
                        ...
                        <p className="text-2xs text-[#4E7568] text-center py-2 bg-[#EEF5E1]/50 rounded-3xl">

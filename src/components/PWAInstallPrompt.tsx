...
export default function PWAInstallPrompt() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-[#0C342C] text-white rounded-3xl shadow-2xl p-3.5 flex items-start gap-3">
          <img
            src="/icon-192.png"
            alt="말씀나눔"
          ...
          <div className="flex-1 min-w-0">
            ...
            {showIosGuide ? (
              <p className="text-2xs text-[#CFE0C2]/90 mt-1 leading-relaxed">
            ...
            {!showIosGuide && (
              <button
                type="button"
                onClick={install}
                className="mt-2 inline-flex items-center gap-1.5 bg-[#0C342C] hover:bg-[#06231D] text-white font-bold text-xs px-3.5 py-1.5 rounded-3xl transition cursor-pointer shadow-sm"
              >
                <Download size={14} />
                <span>앱 설치하기</span>
              </button>
          ...
          <button
            ...
            onClick={dismiss}
            aria-label="닫기"
            className="text-[#CFE0C2]/70 hover:text-white transition cursor-pointer shrink-0 p-0.5"
          >
            <X size={18} />
          </button>
        </div>

...
export default function MeditationFeed({ currentUser, allUsers, prefilledVerse, onClearPrefilledVerse }: MeditationFeedProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-[#0C342C]">묵상 나눔</h2>
        <p className="text-xs sm:text-sm text-[#4E7568] mt-0.5">오늘 받은 은혜를 함께 나눠요</p>
      </div>
      ...
      <AnimatePresence>
        {realtimeToast && (
          <motion.div
            ...
            className="bg-[#0C342C] text-white p-3.5 rounded-3xl shadow-xl flex items-center justify-between gap-3 relative z-30"
          >
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-[#1E6B57] rounded-3xl text-white animate-bounce shrink-0">
                <Bell size={16} />
              </div>
              <p className="text-xs font-semibold leading-relaxed text-[#EEF5E1]">
                {realtimeToast}
              </p>
            </div>
            <button
              onClick={() => setRealtimeToast(null)}
              className="p-1 hover:bg-[#06231D] text-[#7C9A87] hover:text-white rounded-xl transition cursor-pointer"
            >
              <X size={14} />
            </button>
          </motion.div>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none">
        ...
        {currentUser.role === 'admin' && (
          <button
            ...
            className="shrink-0 flex items-center gap-1 px-3.5 py-2 rounded-full text-xs font-bold text-[#4E7568] bg-[#EFF6E2] hover:bg-[#E4EFD1] transition cursor-pointer"
        ...
        {showWriteForm && (
          ...
          >
            <form onSubmit={handleSubmit} className="bg-white rounded-3xl sm:rounded-[32px] shadow-sm p-3.5 sm:p-6 space-y-3.5">
              <h3 className="text-base font-bold text-[#0C342C] flex items-center">
                <PenTool className="mr-1.5 text-[#1E6B57]" size={18} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#4E7568] mb-1">묵상 말씀 구절</label>
                  <input
                    type="text"
                    required
                    value={verseTitle}
                    ...
                    className="w-full text-sm px-3.5 py-3 bg-[#EEF5E1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] text-[#0B2A20] font-semibold"
                  />
                </div>
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-[#4E7568] mb-1">묵상 고백 및 나누고 싶은 내용</label>
                <textarea
                  required
                  rows={6}
                  value={content}
                  ...
                  className="w-full text-sm px-3.5 py-3 bg-[#EEF5E1] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] text-[#0B2A20] leading-relaxed"
                />
                <MentionPicker
              ...
              <div>
                <label className="block text-xs font-semibold text-[#4E7568] mb-1">오늘 나의 기도 제목 (선택)</label>
                <textarea
                  rows={2}
                  value={prayer}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#4E7568] mb-1">
                  나눔 범위 선택 (속 모임 / 전체 공유)
                </label>
                <select
                  value={sokIdForForm || ""}
                  onChange={(e) => setSokIdForForm(e.target.value || null)}
                  className="w-full text-sm px-3.5 py-3 bg-white border-2 border-[#0C342C] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57] text-[#0B2A20] font-bold cursor-pointer"
                >
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold rounded-3xl text-[#1E6B57] bg-white hover:bg-[#EEF5E1] transition cursor-pointer"
                >
                  취소
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2.5 text-sm font-bold rounded-3xl text-white bg-[#1E6B57] hover:bg-[#06231D] shadow-md transition cursor-pointer"
                >
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="bg-white rounded-[32px] p-12 text-center text-[#4E7568]">
            <Clock className="animate-spin mx-auto text-[#1E6B57] mb-2" size={24} />
          ...
          filteredMeditations.map((med) => {
            ...
            return (
              <motion.div
                ...
                className="bg-[#EFF6E2] rounded-[22px] p-5 space-y-3 transition-all duration-300"
              >
                {/* Header — 이름 / 날짜 · 방 배지 (시안의 .post-head) */}
                <div className="flex justify-between items-start gap-2">
                  <div className="min-w-0">
                    {/* 이름은 카드에서 가장 먼저 읽히는 것 — 넉넉히 크게 */}
                    <span className="block text-[0.97rem] font-bold text-[#0B2A20] leading-[1.3]">
                      {med.userName}
                    </span>
                    <span className="flex items-center gap-1.5 mt-1">
                      <span className="text-xs text-[#4E7568]">{shortDate(med.date || med.createdAt)}</span>
                      {med.sokId ? (
                        <span className="text-xs font-bold text-[#1E6B57] bg-[#CFE0C2] px-2 py-0.5 rounded-full">
                  ...
                  {isMyMed && (
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className="p-1.5 text-[#4E7568] hover:text-[#1E6B57] hover:bg-white rounded-xl transition cursor-pointer"
                        title="수정"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
                        ...
                        className="p-1.5 text-[#4E7568] hover:text-[#B3261E] hover:bg-white rounded-xl transition cursor-pointer"
                        title="삭제"
                      >
                        <Trash2 size={14} />
                      </button>
                ...
                {med.verseTitle && (
                  <p className="text-[0.88rem] font-bold text-[#1E6B57]">{med.verseTitle}</p>
                )}

                {/* Content body (제목 없이 본문만) — 앞머리의 골라 온 성경 구절만
                    굵고 진하게 해서 내 묵상과 구별한다. 카드 배경 위에 그대로 놓는다.
                    긴 글은 접어 둔다 — 안 그러면 다음 지체 묵상까지 한참 내려가야 한다. */}
                <CollapsibleText fadeColor="#EFF6E2">
                  {medQuote && (
                    <p className="text-[0.88rem] font-bold text-[#0C342C] leading-[1.6] whitespace-pre-line">
                      {medQuote}
                    </p>
                  )}
                  {medBody && (
                    <MentionText
                      text={medBody}
                      names={memberNames}
                      className={`text-[0.92rem] text-[#0B2A20] leading-[1.62] whitespace-pre-line ${
                        medQuote ? "mt-2" : ""
                      }`}
                    />
                  )}
                ...
                {med.prayer && (
                  <div className="bg-white rounded-3xl p-4 text-[0.88rem] space-y-2">
                    <span className="font-bold text-[#0C342C] block">기도제목</span>
                    <CollapsibleText collapsedHeight={110} fadeColor="#FFFFFF">
                      <MentionText
                        text={`"${med.prayer}"`}
                        names={memberNames}
                        className="text-[#1E6B57] leading-relaxed font-medium italic whitespace-pre-line"
                      />
                    </CollapsibleText>

                    {/* 글쓴이에게만: 몇 명이 함께 기도했는지 */}
                    {isMyMed && (med.reactions?.pray?.length || 0) > 0 && (
                      <p className="text-xs font-bold text-[#0C342C] bg-[#E7F6D8] ring-1 ring-[#C2DAB0] rounded-3xl px-3 py-2">
                ...
                <div className="flex items-center gap-4 pt-1 flex-wrap">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs font-semibold transition cursor-pointer whitespace-nowrap ${
                      commentsOpen ? "text-[#1E6B57]" : "text-[#4E7568] hover:text-[#1E6B57]"
                    }`}
                  >
                            ...
                            <div key={comment.id} className="flex justify-between items-start gap-2 bg-white/80 p-3 rounded-3xl">
                              <div className="text-[0.88rem]">
                                <div className="flex items-center gap-1.5 mb-1">
                                  <strong className="font-bold text-[#0C342C]">{comment.userName}</strong>
                                ...
                                <MentionText
                                  ...
                                  names={memberNames}
                                  className="text-[#1E6B57] leading-relaxed font-medium whitespace-pre-line"
                                />
                              </div>
                              ...
                              {isMyComment && (
                                <button
                                  ...
                                  className="text-[#7C9A87] hover:text-[#B3261E] p-0.5 rounded cursor-pointer"
                                  title="댓글 삭제"
                                >
                                  <X size={12} />
                                </button>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 text-sm px-3.5 py-2.5 bg-[#EEF5E1] rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-[#1E6B57] text-[#0B2A20]"
                        />
                        <button
                          onClick={() => handleAddComment(med.id)}
                          className="px-3 bg-[#1E6B57] hover:bg-[#06231D] text-white rounded-xl transition flex items-center justify-center cursor-pointer"
                        >
                          <Send size={12} />
                        </button>
                      </div>
          ...
          <div className="bg-[#EEF5E1] rounded-[32px] p-12 text-center text-[#4E7568]">
            <BookOpen className="mx-auto text-[#4E7568] mb-2" size={32} />
            <p className="text-sm font-semibold text-[#1E6B57]">아직 조건에 맞는 묵상 나눔 글이 없습니다.</p>
            <p className="text-xs text-[#4E7568] mt-1">지체 중 첫 번째로 묵상 고백을 올려보세요!</p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {showSokManageModal && (
          ...
          >
            ...
            >
              <div className="flex justify-between items-center border-b border-[#DEE9D2] pb-4">
                <div className="flex items-center gap-2">
                  <div className="p-2 bg-[#EEF5E1] text-[#0C342C] rounded-3xl font-bold">
                    <Users size={18} />
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-[#0C342C]">속(소그룹) 관리 및 구성원 설정</h2>
                    <p className="text-xs text-[#4E7568]">속을 추가하고, 이름을 수정하며, 각 속의 모임 멤버를 관리할 수 있습니다.</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowSokManageModal(false)}
                  className="p-1.5 text-[#4E7568] hover:text-[#1E6B57] rounded-xl transition cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              {/* Add New Sok Form */}
              <form onSubmit={handleCreateSok} className="bg-[#E4EFD1] p-4 rounded-3xl space-y-3">
                <h3 className="text-xs font-bold text-[#0C342C] flex items-center gap-1">
                  <Plus size={14} className="text-[#1E6B57]" />
                  새 속(소그룹) 추가
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="text-xs p-2.5 bg-[#EEF5E1] rounded-3xl font-bold focus:outline-none focus:ring-2 focus:ring-[#1E6B57]"
                  />
                  <input
                    type="text"
                    placeholder="속 설명 (선택)"
                    ...
                    className="text-xs p-2.5 bg-[#EEF5E1] rounded-3xl focus:outline-none focus:ring-2 focus:ring-[#1E6B57]"
                  />
                </div>
                <div className="flex justify-end">
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#1E6B57] hover:bg-[#06231D] text-white font-bold text-xs rounded-3xl shadow-xs transition cursor-pointer flex items-center gap-1"
                  >
                    <Plus size={14} />
                    속 추가하기
                  </button>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-[#0C342C]">개설된 속 목록 ({sokGroups.length}개)</h3>
                {sokGroups.length === 0 ? (
                  <p className="text-xs text-[#4E7568] py-4 text-center bg-[#EEF5E1] rounded-3xl">
                  ...
                  sokGroups.map((sok) => {
                    ...
                    return (
                      <div key={sok.id} className="bg-[#EEF5E1] rounded-3xl p-4 space-y-3 shadow-xs">
                        {isEditing ? (
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              <input
                                ...
                                className="text-xs p-2 bg-[#EEF5E1] rounded-xl font-bold flex-1"
                                placeholder="속 이름"
                              />
                              <input
                                type="text"
                                ...
                                className="text-xs p-2 bg-[#EEF5E1] rounded-xl flex-1"
                                placeholder="설명"
                              />
                            </div>
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="px-3 py-1 text-xs bg-[#EEF5E1] rounded-xl cursor-pointer"
                              >
                                취소
                              </button>
                              <button
                                ...
                                className="px-3 py-1 text-xs bg-[#1E6B57] text-white font-bold rounded-xl cursor-pointer"
                              >
                                저장
                              </button>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-bold text-sm text-[#0C342C]">{sok.name}</span>
                                <span className="text-2xs bg-[#EEF5E1] text-[#0C342C] px-2 py-0.5 rounded-full font-semibold">
                                  구성원 {membersCount}명
                                </span>
                              </div>
                              {sok.description && (
                                <p className="text-xs text-[#4E7568] mt-0.5">{sok.description}</p>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className={`px-2.5 py-1 text-xs font-bold rounded-3xl transition flex items-center gap-1 cursor-pointer ${
                                  isMemberSettingsOpen
                                    ...
                                    : "bg-[#EEF5E1] hover:bg-[#EEF5E1] text-[#0C342C]"
                                }`}
                              >
                                <UserPlus size={13} />
                                <span>구성원 지정</span>
                              ...
                              <button
                                ...
                                className="p-1.5 text-[#4E7568] hover:text-[#1E6B57] hover:bg-[#EEF5E1] rounded-xl transition cursor-pointer"
                                title="이름 수정"
                              >
                                <Edit3 size={14} />
                              </button>
                              <button
                                ...
                                className="p-1.5 text-[#4E7568] hover:text-[#B3261E] hover:bg-[#FDF3F3] rounded-xl transition cursor-pointer"
                                title="속 삭제"
                              >
                                <Trash2 size={14} />
                              </button>
                        ...
                        {isMemberSettingsOpen && (
                          <div className="bg-[#EEF5E1] rounded-3xl p-3 space-y-2.5 mt-2">
                            <div className="flex items-center justify-between text-xs font-bold text-[#0C342C]">
                              <span>속 구성원 지정 ({sok.name})</span>
                              <span className="text-2xs text-[#4E7568] font-normal">
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex items-center gap-2 p-2 rounded-3xl text-xs font-semibold cursor-pointer select-none transition ${
                                      isMember
                                        ? "bg-white text-[#0C342C] shadow-xs"
                                        : "bg-white/50 text-[#4E7568] hover:bg-white"
                                    }`}
                                  >
                                    <div
                                      className={`w-4 h-4 rounded-lg flex items-center justify-center border transition ${
                                        isMember ? "bg-[#1E6B57] border-[#1E6B57] text-white" : "border-[#7C9A87]"
                                      }`}
                                    >
              ...
              <div className="flex justify-end pt-2 border-t border-[#DEE9D2]">
                <button
                  onClick={() => setShowSokManageModal(false)}
                  className="px-5 py-2 bg-[#0C342C] text-white font-bold text-xs rounded-3xl hover:bg-[#06231D] transition cursor-pointer"
                >
                  닫기
                </button>
              </div>

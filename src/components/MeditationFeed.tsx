...
export default function MeditationFeed({ currentUser, allUsers, prefilledVerse, onClearPrefilledVerse }: MeditationFeedProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <AnimatePresence>
        {realtimeToast && (
          <motion.div
            ...
            className="bg-[#2C2F36] text-white p-3.5 rounded-2xl shadow-xl border border-[#E3E4E7] flex items-center justify-between gap-3 relative z-30"
          >
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-[#4B4E55] rounded-2xl text-white animate-bounce shrink-0">
                <Bell size={16} />
              </div>
              <p className="text-xs font-semibold leading-relaxed text-[#EDEEF0]">
                {realtimeToast}
              </p>
            </div>
            <button
              onClick={() => setRealtimeToast(null)}
              className="p-1 hover:bg-[#1E2128] text-[#C3C5CA] hover:text-white rounded-xl transition cursor-pointer"
            >
              <X size={14} />
            </button>
          </motion.div>
      ...
      <div className="bg-[#F4F5F7] border border-[#E3E4E7] rounded-2xl px-3.5 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2 text-[#2C2F36] font-bold">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#85888F] opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#2C2F36]"></span>
          </span>
          <span className="text-2xs sm:text-xs">실시간 나눔 자동 반영 중</span>
          <span className="text-2xs text-[#85888F] font-normal hidden sm:inline">(지체들의 글과 댓글이 4초마다 자동 동기화됩니다)</span>
        </div>

        {notiPermission !== "granted" ? (
          <button
            onClick={requestNotificationPermission}
            className="flex items-center gap-1 text-2xs font-bold text-[#4B4E55] bg-white hover:bg-[#EDEEF0]/50 border border-[#4B4E55]/30 px-2.5 py-1 rounded-2xl transition cursor-pointer"
          >
            <Bell size={12} className="animate-pulse" />
            새 묵상 스마트폰/브라우저 알림 켜기
          </button>
        ) : (
          <span className="flex items-center gap-1 text-2xs font-bold text-[#2C2F36] bg-[#F4F5F7] border border-[#E3E4E7] px-2 py-0.5 rounded-xl">
            <CheckCircle size={11} />
            알림 수신 중
          </span>
        )}
      ...
      <div className="bg-[#F4F5F7] p-2 rounded-2xl border border-[#E3E4E7] flex flex-wrap items-center justify-between gap-2 shadow-xs">
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5 max-w-full">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-2xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
              selectedSokTab === "all"
                ? "bg-[#2C2F36] text-white shadow-xs"
                : "bg-white text-[#4B4E55] hover:bg-[#E3E4E7] border border-[#E3E4E7]"
            }`}
          >
            <Globe size={13} />
            <span>전체 공유</span>
          ...
          {accessibleSoks.map((sok) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-2xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
                  isSelected
                    ? "bg-[#4B4E55] text-white shadow-xs"
                    : "bg-white text-[#2C2F36] hover:bg-[#EDEEF0] border border-[#E3E4E7]"
                }`}
              >
                <Users size={13} />
                <span>{sok.name}</span>
                {sok.memberUserIds?.length > 0 && (
                  <span className={`text-2xs px-1.5 py-0.2 rounded-full ${isSelected ? "bg-white/20 text-white" : "bg-[#F4F5F7] text-[#4B4E55]"}`}>
        ...
        {currentUser.role === 'admin' && (
          <button
            ...
            className="px-3 py-1.5 bg-[#2C2F36] hover:bg-[#1E2128] text-white rounded-2xl text-xs font-bold transition flex items-center gap-1 cursor-pointer shrink-0 shadow-xs"
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-[#E3E4E7] shadow-sm p-3 sm:p-4 flex flex-col md:flex-row md:items-center justify-between gap-2.5">
        <div className="flex flex-1 flex-col sm:flex-row gap-2">
          ...
          <select
            ...
            className="px-2.5 py-2 bg-[#F4F5F7] border border-[#E3E4E7] rounded-2xl focus:outline-none text-[#1A1C21] text-xs font-semibold cursor-pointer"
          >
          ...
          {/* Date Picker Filter */}
          <div className="flex items-center gap-1 bg-[#F4F5F7] border border-[#E3E4E7] rounded-2xl px-2">
            <input
              type="date"
              value={selectedDateFilter}
              onChange={(e) => setSelectedDateFilter(e.target.value)}
              className="bg-transparent border-none text-[#1A1C21] text-xs font-semibold py-2 focus:outline-none cursor-pointer"
              title="날짜별 검색"
            />
            {selectedDateFilter && (
              <button
                onClick={() => setSelectedDateFilter("")}
                className="text-[#85888F] hover:text-[#4B4E55] p-1 cursor-pointer"
                title="날짜 선택 해제"
              >
                <X size={14} />
              </button>
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-[#4B4E55] hover:bg-[#1E2128] text-white font-bold text-xs px-3.5 py-2 rounded-2xl shadow-md transition cursor-pointer whitespace-nowrap shrink-0"
        >
      ...
      <AnimatePresence>
        {showWriteForm && (
          ...
          >
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl sm:rounded-3xl border border-[#E3E4E7] shadow-sm p-3.5 sm:p-6 space-y-3.5">
              <h3 className="text-base font-bold text-[#2C2F36] flex items-center">
                <PenTool className="mr-1.5 text-[#4B4E55]" size={18} />
                {editingId ? "내 묵상 글 수정하기" : "오늘의 묵상 글 작성"}
              </h3>

              {formError && (
                <div className="bg-[#FFF2F2] border border-[#FFE8E8] text-[#C40000] text-xs p-3 rounded-2xl flex items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#85888F] mb-1">묵상 말씀 구절</label>
                  <input
                    type="text"
                    required
                    value={verseTitle}
                    ...
                    className="w-full text-xs px-3 py-2.5 border border-[#E3E4E7] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4B4E55] text-[#1A1C21] font-semibold"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-[#85888F] mb-1">묵상 글 제목</label>
                  <input
                    type="text"
                    required
                    value={title}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#85888F] mb-1">묵상 고백 및 나누고 싶은 내용</label>
                <textarea
                  required
                  rows={6}
                  value={content}
                  ...
                  className="w-full text-xs px-3 py-2.5 border border-[#E3E4E7] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4B4E55] text-[#1A1C21] leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#85888F] mb-1">오늘 나의 기도 제목 (선택)</label>
                <textarea
                  rows={2}
                  value={prayer}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#85888F] mb-1">
                  나눔 범위 선택 (속 모임 / 전체 공유)
                </label>
                <select
                  value={sokIdForForm || ""}
                  onChange={(e) => setSokIdForForm(e.target.value || null)}
                  className="w-full text-xs px-3 py-2.5 border border-[#E3E4E7] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4B4E55] text-[#1A1C21] font-bold bg-[#FFFFFF] cursor-pointer"
                >
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button
                  ...
                  className="px-4 py-2 border border-[#E3E4E7] text-xs font-semibold rounded-2xl text-[#4B4E55] bg-white hover:bg-[#F4F5F7] transition cursor-pointer"
                >
                  취소
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 text-xs font-bold rounded-2xl text-white bg-[#4B4E55] hover:bg-[#1E2128] shadow-md transition cursor-pointer"
                >
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="bg-white rounded-3xl border border-[#E3E4E7] p-12 text-center text-[#85888F]">
            <Clock className="animate-spin mx-auto text-[#4B4E55] mb-2" size={24} />
          ...
          filteredMeditations.map((med) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white rounded-3xl border border-[#E3E4E7] shadow-sm p-5 space-y-4 hover:border-[#4B4E55]/50 hover:shadow-md transition-all duration-300"
              >
                {/* Header info */}
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-[#EDEEF0] flex items-center justify-center font-bold text-[#2C2F36] border border-[#E3E4E7] text-sm">
                      {med.userName.slice(-2)}
                    </div>
                    <div>
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="font-bold text-[#FF0000] text-sm">{med.userName}</span>
                        <span className="text-2xs bg-[#F4F5F7] text-[#4B4E55] px-2 py-0.5 rounded-full font-bold">
                          {med.verseTitle}
                        </span>
                        {med.sokId ? (
                          <span className="text-2xs bg-[#EDEEF0] text-[#2C2F36] border border-[#DCDEE1] px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                            <Users size={11} />
                            {sokGroups.find(s => s.id === med.sokId)?.name || "속 나눔"}
                          </span>
                        ) : (
                          <span className="text-2xs bg-[#F4F5F7] text-[#85888F] border border-[#E3E4E7] px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                            <Globe size={11} />
                            전체 공유
                          </span>
                        )}
                  ...
                  {isMyMed && (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 text-[#85888F] hover:text-[#4B4E55] hover:bg-[#F4F5F7] rounded-xl transition cursor-pointer"
                        title="수정"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
                        ...
                        className="p-1.5 text-[#85888F] hover:text-[#FF0000] hover:bg-[#F4F5F7] rounded-xl transition cursor-pointer"
                        title="삭제"
                      >
                        <Trash2 size={14} />
                      </button>
                ...
                <div className="space-y-2">
                  <h4 className="text-base font-bold text-[#2C2F36]">{med.title}</h4>

                  <p className="text-sm text-[#4B4E55] leading-relaxed whitespace-pre-line bg-[#EFF1F3] p-4 rounded-2xl border border-[#E3E4E7]">
                    {med.content}
                  </p>
                </div>
                ...
                {med.prayer && (
                  <div className="bg-[#EDEEF0]/40 border border-[#E3E4E7] rounded-2xl p-4 text-xs">
                    <span className="font-bold text-[#2C2F36] block mb-1">🙏 이번 주 동역자 기도제목</span>
                    <p className="text-[#4B4E55] leading-relaxed font-medium italic">
                      &quot;{med.prayer}&quot;
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between border-t border-[#E3E4E7] pt-3">
                  <div className="flex items-center gap-4">
                    {/* Like button */}
                    <button
                      onClick={() => handleLikeToggle(med.id)}
                      className={`flex items-center gap-1.5 text-xs font-bold transition cursor-pointer ${
                        hasLiked ? "text-[#FF0000]" : "text-[#85888F] hover:text-[#FF0000]"
                      }`}
                    >
                      <Heart size={16} className={hasLiked ? "fill-[#FF0000] text-[#FF0000]" : ""} />
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 text-xs font-bold transition cursor-pointer ${
                        commentsOpen ? "text-[#2C2F36]" : "text-[#85888F] hover:text-[#2C2F36]"
                      }`}
                    >
                  ...
                  {med.likes.length > 0 && (
                    <div className="text-2xs text-[#85888F] font-medium">
                ...
                {commentsOpen && (
                  <div className="bg-[#F4F5F7]/60 border border-[#E3E4E7] rounded-2xl p-4 space-y-3 mt-2">
                    ...
                    {med.comments.length > 0 ? (
                      <div className="space-y-2.5">
                        {med.comments.map((comment) => {
                          ...
                          return (
                            <div key={comment.id} className="flex justify-between items-start gap-2 bg-white/80 p-3 rounded-2xl border border-[#E3E4E7]">
                              <div className="text-xs">
                                <div className="flex items-center gap-1.5 mb-1">
                                  <strong className="font-bold text-[#FF0000]">{comment.userName}</strong>
                                  <span className="text-2xs text-[#85888F]">
                                    {new Date(comment.createdAt).toLocaleDateString()}
                                  </span>
                                </div>
                                <p className="text-[#4B4E55] leading-relaxed font-medium">
                                  {comment.content}
                                </p>
                              </div>
                              ...
                              {isMyComment && (
                                <button
                                  ...
                                  className="text-[#C3C5CA] hover:text-[#FF0000] p-0.5 rounded cursor-pointer"
                                  title="댓글 삭제"
                                >
                                  <X size={12} />
                                </button>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 text-xs px-3 py-2 border border-[#E3E4E7] rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-[#4B4E55] text-[#1A1C21]"
                      />
                      <button
                        onClick={() => handleAddComment(med.id)}
                        className="px-3 bg-[#4B4E55] hover:bg-[#1E2128] text-white rounded-xl transition flex items-center justify-center cursor-pointer"
                      >
                        <Send size={12} />
                      </button>
                    </div>
          ...
          <div className="bg-[#F4F5F7] rounded-3xl p-12 text-center border border-dashed border-[#E3E4E7] text-[#85888F]">
            <BookOpen className="mx-auto text-[#85888F] mb-2" size={32} />
            <p className="text-sm font-semibold text-[#4B4E55]">아직 조건에 맞는 묵상 나눔 글이 없습니다.</p>
            <p className="text-xs text-[#85888F] mt-1">지체 중 첫 번째로 묵상 고백을 올려보세요!</p>
          </div>
        )}
      </div>
        ...
        {showSokManageModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 shadow-2xl border border-[#E3E4E7]"
            >
              <div className="flex justify-between items-center border-b border-[#E3E4E7] pb-4">
                <div className="flex items-center gap-2">
                  <div className="p-2 bg-[#EDEEF0] text-[#2C2F36] rounded-2xl font-bold">
                    <Users size={18} />
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-[#2C2F36]">속(소그룹) 관리 및 구성원 설정</h2>
                    <p className="text-xs text-[#85888F]">속을 추가하고, 이름을 수정하며, 각 속의 모임 멤버를 관리할 수 있습니다.</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowSokManageModal(false)}
                  className="p-1.5 text-[#85888F] hover:text-[#4B4E55] rounded-xl transition cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              {/* Add New Sok Form */}
              <form onSubmit={handleCreateSok} className="bg-[#EFF1F3] border border-[#E3E4E7] p-4 rounded-2xl space-y-3">
                <h3 className="text-xs font-bold text-[#2C2F36] flex items-center gap-1">
                  <Plus size={14} className="text-[#4B4E55]" />
                  새 속(소그룹) 추가
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="text-xs p-2.5 bg-white border border-[#E3E4E7] rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-[#4B4E55]"
                  />
                  <input
                    type="text"
                    placeholder="속 설명 (선택)"
                    ...
                    className="text-xs p-2.5 bg-white border border-[#E3E4E7] rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#4B4E55]"
                  />
                </div>
                <div className="flex justify-end">
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#4B4E55] hover:bg-[#1E2128] text-white font-bold text-xs rounded-2xl shadow-xs transition cursor-pointer flex items-center gap-1"
                  >
                    <Plus size={14} />
                    속 추가하기
                  </button>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-[#2C2F36]">개설된 속 목록 ({sokGroups.length}개)</h3>
                {sokGroups.length === 0 ? (
                  <p className="text-xs text-[#85888F] py-4 text-center border border-dashed border-[#E3E4E7] rounded-2xl">
                  ...
                  sokGroups.map((sok) => {
                    ...
                    return (
                      <div key={sok.id} className="bg-white border border-[#E3E4E7] rounded-2xl p-4 space-y-3 shadow-xs">
                        {isEditing ? (
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              <input
                                ...
                                className="text-xs p-2 border border-[#E3E4E7] rounded-xl font-bold flex-1"
                                placeholder="속 이름"
                              />
                              <input
                                type="text"
                                ...
                                className="text-xs p-2 border border-[#E3E4E7] rounded-xl flex-1"
                                placeholder="설명"
                              />
                            </div>
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="px-3 py-1 text-xs border border-[#E3E4E7] rounded-xl cursor-pointer"
                              >
                                취소
                              </button>
                              <button
                                ...
                                className="px-3 py-1 text-xs bg-[#4B4E55] text-white font-bold rounded-xl cursor-pointer"
                              >
                                저장
                              </button>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-bold text-sm text-[#2C2F36]">{sok.name}</span>
                                <span className="text-2xs bg-[#EDEEF0] text-[#2C2F36] px-2 py-0.5 rounded-full font-semibold">
                                  구성원 {membersCount}명
                                </span>
                              </div>
                              {sok.description && (
                                <p className="text-xs text-[#85888F] mt-0.5">{sok.description}</p>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className={`px-2.5 py-1 text-xs font-bold rounded-2xl transition flex items-center gap-1 cursor-pointer ${
                                  isMemberSettingsOpen
                                    ...
                                    : "bg-[#F4F5F7] hover:bg-[#EDEEF0] text-[#2C2F36]"
                                }`}
                              >
                                <UserPlus size={13} />
                                <span>구성원 지정</span>
                              ...
                              <button
                                ...
                                className="p-1.5 text-[#85888F] hover:text-[#FF0000] hover:bg-[#FFF2F2] rounded-xl transition cursor-pointer"
                                title="속 삭제"
                              >
                                <Trash2 size={14} />
                              </button>
                        ...
                        {isMemberSettingsOpen && (
                          <div className="bg-[#EFF1F3] border border-[#E3E4E7] rounded-2xl p-3 space-y-2.5 mt-2">
                            <div className="flex items-center justify-between text-xs font-bold text-[#2C2F36]">
                              <span>속 구성원 지정 ({sok.name})</span>
                              <span className="text-2xs text-[#85888F] font-normal">
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex items-center gap-2 p-2 rounded-2xl text-xs font-semibold border cursor-pointer select-none transition ${
                                      isMember
                                        ? "bg-white border-[#4B4E55] text-[#2C2F36] shadow-xs"
                                        : "bg-white/50 border-[#E3E4E7] text-[#85888F] hover:bg-white"
                                    }`}
                                  >
                                    <div
                                      className={`w-4 h-4 rounded-md flex items-center justify-center border transition ${
                                        isMember ? "bg-[#4B4E55] border-[#4B4E55] text-white" : "border-[#C3C5CA]"
                                      }`}
                                    >
              ...
              <div className="flex justify-end pt-2 border-t border-[#E3E4E7]">
                <button
                  onClick={() => setShowSokManageModal(false)}
                  className="px-5 py-2 bg-[#2C2F36] text-white font-bold text-xs rounded-2xl hover:bg-[#1E2128] transition cursor-pointer"
                >
                  닫기
                </button>
              </div>

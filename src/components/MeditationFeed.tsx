...
export default function MeditationFeed({ currentUser, allUsers, prefilledVerse, onClearPrefilledVerse }: MeditationFeedProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <AnimatePresence>
        {realtimeToast && (
          <motion.div
            ...
            className="bg-[#14603F] text-white p-3.5 rounded-2xl shadow-xl border border-[#E5E5EA] flex items-center justify-between gap-3 relative z-30"
          >
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-[#17784C] rounded-2xl text-white animate-bounce shrink-0">
                <Bell size={16} />
              </div>
            ...
            <button
              ...
              className="p-1 hover:bg-[#125E3B] text-slate-300 hover:text-white rounded-xl transition cursor-pointer"
            >
              <X size={14} />
            </button>
          </motion.div>
      ...
      <div className="bg-[#F2F2F7] border border-[#E5E5EA] rounded-2xl px-3.5 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2 text-[#14603F] font-bold">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-600"></span>
          </span>
          <span className="text-[11px] sm:text-xs">실시간 나눔 자동 반영 중</span>
          <span className="text-[10px] text-[#8E8E93] font-normal hidden sm:inline">(지체들의 글과 댓글이 4초마다 자동 동기화됩니다)</span>
        </div>

        {notiPermission !== "granted" ? (
          <button
            onClick={requestNotificationPermission}
            className="flex items-center gap-1 text-[11px] font-bold text-[#17784C] bg-white hover:bg-[#E3F2E9]/50 border border-[#17784C]/30 px-2.5 py-1 rounded-2xl transition cursor-pointer"
          >
            <Bell size={12} className="animate-pulse" />
            새 묵상 스마트폰/브라우저 알림 켜기
          </button>
        ) : (
          <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-xl">
            <CheckCircle size={11} />
            알림 수신 중
          </span>
        )}
      ...
      <div className="bg-[#F2F2F7] p-2 rounded-2xl border border-[#E5E5EA] flex flex-wrap items-center justify-between gap-2 shadow-xs">
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5 max-w-full">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-2xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
              selectedSokTab === "all"
                ? "bg-[#14603F] text-white shadow-xs"
                : "bg-white text-[#48484A] hover:bg-[#E5E5EA] border border-[#E5E5EA]"
            }`}
          >
            <Globe size={13} />
            <span>전체 공유</span>
          ...
          {accessibleSoks.map((sok) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-2xl text-xs font-bold transition flex items-center gap-1.5 whitespace-nowrap cursor-pointer ${
                  isSelected
                    ? "bg-[#17784C] text-white shadow-xs"
                    : "bg-white text-[#14603F] hover:bg-[#E3F2E9] border border-[#E5E5EA]"
                }`}
              >
                <Users size={13} />
                <span>{sok.name}</span>
                {sok.memberUserIds?.length > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isSelected ? "bg-white/20 text-white" : "bg-[#F2F2F7] text-[#17784C]"}`}>
        ...
        {currentUser.role === 'admin' && (
          <button
            ...
            className="px-3 py-1.5 bg-[#14603F] hover:bg-[#0E4A30] text-white rounded-2xl text-xs font-bold transition flex items-center gap-1 cursor-pointer shrink-0 shadow-xs"
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-[#E5E5EA] shadow-sm p-3 sm:p-4 flex flex-col md:flex-row md:items-center justify-between gap-2.5">
        <div className="flex flex-1 flex-col sm:flex-row gap-2">
          ...
          <select
            ...
            className="px-2.5 py-2 bg-[#F2F2F7] border border-[#E5E5EA] rounded-2xl focus:outline-none text-slate-800 text-xs font-semibold cursor-pointer"
          >
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-[#17784C] hover:bg-[#125E3B] text-white font-bold text-xs px-3.5 py-2 rounded-2xl shadow-md transition cursor-pointer whitespace-nowrap shrink-0"
        >
      ...
      <AnimatePresence>
        {showWriteForm && (
          ...
          >
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl sm:rounded-3xl border border-[#E5E5EA] shadow-sm p-3.5 sm:p-6 space-y-3.5">
              <h3 className="text-base font-bold text-[#14603F] flex items-center">
                <PenTool className="mr-1.5 text-[#17784C]" size={18} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#8E8E93] mb-1">묵상 말씀 구절</label>
                  <input
                    type="text"
                    required
                    value={verseTitle}
                    ...
                    className="w-full text-xs px-3 py-2.5 border border-[#E5E5EA] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#17784C] text-slate-800 font-semibold"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-[#8E8E93] mb-1">묵상 글 제목</label>
                  <input
                    type="text"
                    required
                    value={title}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#8E8E93] mb-1">묵상 고백 및 나누고 싶은 내용</label>
                <textarea
                  required
                  rows={6}
                  value={content}
                  ...
                  className="w-full text-xs px-3 py-2.5 border border-[#E5E5EA] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#17784C] text-slate-800 leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#8E8E93] mb-1">오늘 나의 기도 제목 (선택)</label>
                <textarea
                  rows={2}
                  value={prayer}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full text-xs px-3 py-2.5 border border-[#E5E5EA] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#17784C] text-slate-800 font-bold bg-[#FFFFFF] cursor-pointer"
                >
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button
                  ...
                  className="px-4 py-2 border border-[#E5E5EA] text-xs font-semibold rounded-2xl text-slate-600 bg-white hover:bg-slate-50 transition cursor-pointer"
                >
                  취소
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 text-xs font-bold rounded-2xl text-white bg-[#17784C] hover:bg-[#125E3B] shadow-md transition cursor-pointer"
                >
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="bg-white rounded-3xl border border-[#E5E5EA] p-12 text-center text-[#8E8E93]">
            <Clock className="animate-spin mx-auto text-[#17784C] mb-2" size={24} />
          ...
          filteredMeditations.map((med) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white rounded-3xl border border-[#E5E5EA] shadow-sm p-5 space-y-4 hover:border-[#17784C]/50 hover:shadow-md transition-all duration-300"
              >
                {/* Header info */}
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-[#E3F2E9] flex items-center justify-center font-bold text-[#14603F] border border-[#E5E5EA] text-sm">
                      {med.userName.slice(-2)}
                    </div>
                    <div>
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="font-bold text-[#14603F] text-sm">{med.userName}</span>
                        <span className="text-[10px] bg-[#F2F2F7] text-[#17784C] px-2 py-0.5 rounded-full font-bold">
                          {med.verseTitle}
                        </span>
                        {med.sokId ? (
                          <span className="text-[10px] bg-[#E2F1E8] text-[#14603F] border border-[#C9E5D5] px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                            <Users size={11} />
                            {sokGroups.find(s => s.id === med.sokId)?.name || "속 나눔"}
                          </span>
                        ) : (
                          <span className="text-[10px] bg-[#F2F2F7] text-[#8E8E93] border border-[#E5E5EA] px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                            <Globe size={11} />
                            전체 공유
                          </span>
                        )}
                ...
                <div className="space-y-2">
                  <h4 className="text-base font-bold text-[#14603F]">{med.title}</h4>

                  <p className="text-sm text-[#3A3A3C] leading-relaxed whitespace-pre-line bg-[#F7F7FA] p-4 rounded-2xl border border-[#E5E5EA]">
                    {med.content}
                  </p>
                </div>
                ...
                {med.prayer && (
                  <div className="bg-[#E3F2E9]/40 border border-[#E5E5EA] rounded-2xl p-4 text-xs">
                  ...
                  <div className="bg-[#F2F2F7]/60 border border-[#E5E5EA] rounded-2xl p-4 space-y-3 mt-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 text-xs px-3 py-2 border border-[#E5E5EA] rounded-xl bg-white focus:outline-none focus:ring-1 focus:ring-[#17784C] text-slate-800"
                      />
                      <button
          ...
          <div className="bg-[#F2F2F7] rounded-3xl p-12 text-center border border-dashed border-[#E5E5EA] text-[#8E8E93]">
            <BookOpen className="mx-auto text-[#8E8E93] mb-2" size={32} />
            <p className="text-sm font-semibold text-[#3A3A3C]">아직 조건에 맞는 묵상 나눔 글이 없습니다.</p>
            <p className="text-xs text-[#8E8E93] mt-1">지체 중 첫 번째로 묵상 고백을 올려보세요!</p>
          </div>
        )}
      </div>
        ...
        {showSokManageModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 shadow-2xl border border-[#E5E5EA]"
            >
              <div className="flex justify-between items-center border-b border-[#E5E5EA] pb-4">
                <div className="flex items-center gap-2">
                  <div className="p-2 bg-[#E3F2E9] text-[#14603F] rounded-2xl font-bold">
                    <Users size={18} />
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-[#14603F]">속(소그룹) 관리 및 구성원 설정</h2>
                    <p className="text-xs text-[#8E8E93]">속을 추가하고, 이름을 수정하며, 각 속의 모임 멤버를 관리할 수 있습니다.</p>
                  </div>
                </div>
                <button
              ...
              {/* Add New Sok Form */}
              <form onSubmit={handleCreateSok} className="bg-[#F7F7FA] border border-[#E5E5EA] p-4 rounded-2xl space-y-3">
                <h3 className="text-xs font-bold text-[#14603F] flex items-center gap-1">
                  <Plus size={14} className="text-[#17784C]" />
                  새 속(소그룹) 추가
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="text-xs p-2.5 bg-white border border-[#E5E5EA] rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-[#17784C]"
                  />
                  <input
                    type="text"
                    placeholder="속 설명 (선택)"
                    ...
                    className="text-xs p-2.5 bg-white border border-[#E5E5EA] rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#17784C]"
                  />
                </div>
                <div className="flex justify-end">
                  <button
                    type="submit"
                    className="px-4 py-2 bg-[#17784C] hover:bg-[#125E3B] text-white font-bold text-xs rounded-2xl shadow-xs transition cursor-pointer flex items-center gap-1"
                  >
                    <Plus size={14} />
                    속 추가하기
                  </button>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-[#14603F]">개설된 속 목록 ({sokGroups.length}개)</h3>
                {sokGroups.length === 0 ? (
                  <p className="text-xs text-slate-400 py-4 text-center border border-dashed border-[#E5E5EA] rounded-2xl">
                  ...
                  sokGroups.map((sok) => {
                    ...
                    return (
                      <div key={sok.id} className="bg-white border border-[#E5E5EA] rounded-2xl p-4 space-y-3 shadow-xs">
                        {isEditing ? (
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              <input
                                ...
                                className="text-xs p-2 border border-[#E5E5EA] rounded-xl font-bold flex-1"
                                placeholder="속 이름"
                              />
                              <input
                                type="text"
                                ...
                                className="text-xs p-2 border border-[#E5E5EA] rounded-xl flex-1"
                                placeholder="설명"
                              />
                            </div>
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="px-3 py-1 text-xs border border-[#E5E5EA] rounded-xl cursor-pointer"
                              >
                                취소
                              </button>
                              <button
                                ...
                                className="px-3 py-1 text-xs bg-[#17784C] text-white font-bold rounded-xl cursor-pointer"
                              >
                                저장
                              </button>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-bold text-sm text-[#14603F]">{sok.name}</span>
                                <span className="text-[11px] bg-[#E3F2E9] text-[#14603F] px-2 py-0.5 rounded-full font-semibold">
                                  구성원 {membersCount}명
                                </span>
                              </div>
                              {sok.description && (
                                <p className="text-xs text-[#8E8E93] mt-0.5">{sok.description}</p>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className={`px-2.5 py-1 text-xs font-bold rounded-2xl transition flex items-center gap-1 cursor-pointer ${
                                  isMemberSettingsOpen
                                    ? "bg-[#14603F] text-white"
                                    : "bg-[#F2F2F7] hover:bg-[#E3F2E9] text-[#14603F]"
                                }`}
                              >
                                <UserPlus size={13} />
                                <span>구성원 지정</span>
                              ...
                              <button
                                ...
                                className="p-1.5 text-slate-500 hover:text-[#17784C] hover:bg-[#F2F2F7] rounded-xl transition cursor-pointer"
                                title="이름 수정"
                              >
                                <Edit3 size={14} />
                              </button>
                        ...
                        {isMemberSettingsOpen && (
                          <div className="bg-[#F7F7FA] border border-[#E5E5EA] rounded-2xl p-3 space-y-2.5 mt-2">
                            <div className="flex items-center justify-between text-xs font-bold text-[#14603F]">
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex items-center gap-2 p-2 rounded-2xl text-xs font-semibold border cursor-pointer select-none transition ${
                                      isMember
                                        ? "bg-white border-[#17784C] text-[#14603F] shadow-xs"
                                        : "bg-white/50 border-[#E5E5EA] text-slate-500 hover:bg-white"
                                    }`}
                                  >
                                    <div
                                      className={`w-4 h-4 rounded-md flex items-center justify-center border transition ${
                                        isMember ? "bg-[#17784C] border-[#17784C] text-white" : "border-slate-300"
                                      }`}
                                    >
              ...
              <div className="flex justify-end pt-2 border-t border-[#E5E5EA]">
                <button
                  onClick={() => setShowSokManageModal(false)}
                  className="px-5 py-2 bg-[#14603F] text-white font-bold text-xs rounded-2xl hover:bg-[#0E4A30] transition cursor-pointer"
                >
                  닫기
                </button>
              </div>
